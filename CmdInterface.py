...
def print_summary(summary: SummaryInfo):
    ...
    if inputs:
        Utils.show_title(msg_title='[{}] INPUT PARAMETERS AND STATS'.format(prefix), title_color=ui.brown)

        inputs_lines = inputs.splitlines()
        for line in inputs_lines:
            msg_info = Utils.insert_ui(text=line, highlight_color=ui.lightgray)
            ui.info(ui.red, '  => ', ui.white, ui.faint, *msg_info)

    if real_lines:
        Utils.show_title(msg_title='[{}] PROCESSING STATUS'.format(prefix), title_color=ui.brown)
    ...
    if warnings:
        Utils.show_title(msg_title='[{}] WARNINGS'.format(prefix), title_color=ui.brown)
        warning_lines = warnings.splitlines()
        for line in warning_lines:
            msg_info = Utils.insert_ui(text=line)
            ui.info(*msg_info)

    if errors:
        Utils.show_title(msg_title='[{}] ERRORS'.format(prefix), title_color=ui.brown)
...
class CmdInterface(InterfaceApp):
    ...
    def print_errors(self):
        if self.errors:
            for err in self.errors:
                msg_info = Utils.insert_ui(text=err)
    ...
    def print_input_summary(self):
        ...
        Utils.show_title(msg_title='INPUT PARAMETERS', title_color=ui.darkred)
        print(lines)
    ...
    def get_linkage_grid_by_model(self, linkage_file_name: str = 'linkage_in.shp'):
        ...
        for msg in msgs_it:
            ...
            msg_info = Utils.insert_ui(text=msg)
            ui.info(ui.green, ui.bold, '=> ', ui.white, ui.faint, *msg_info)
    ...
    def check_args(self):
        ...
        my_parser = argparse.ArgumentParser(
            description='Herramienta que genera un archivo de acople (shapefile)'
                        ' entre un modelo hidrológico en WEAP y un modelo hidrogeológico de MODFLOW 2005', allow_abbrev=False)
...
def main(location: str):
    ...
    if not interface_app.check_errors():
        ...
        tmp_location = Utils.generate_word(length=5, prefix='loc_')
        # tmp_location = interface_app.location
        # with Session(gisdb="/home/pc/gisdata/grassdata", location="test", mapset='pc', create_opts=""):
        with Session(gisdb=interface_app.gidsb, location=interface_app.location, create_opts="EPSG:{}".format(interface_app.epsg_code)) as sess:
